...
class Search {
  ...
  async getResults() {
    Promise.all([await fetch(`${univData.root_url}/wp-json/wp/v2/pages?search=${(this, this.searchField.value)}`).then(res => res.json()), await fetch(`${univData.root_url}/wp-json/wp/v2/posts?search=${(this, this.searchField.value)}`).then(re => re.json())]).then(async res => {
