...
class Search {
  ...
  async getResults() {
    fetch(`${univData.root_url}/wp-json/univ/v1/search?term=${this.searchField.value}`).then(res => res.json()).then(res => {
      this.results.innerHTML = `
          <div class="row">
            <div class="one-third">
              <h2 class="search-overlay__section-title">General Information</h2>
              ${res.generalInfo.length ? '<ul class="link-list min-list">' : "<p>No general information found</p>"}
          
          ${res.generalInfo.map(item => `<li><a href="${item.permalink}">${item.title}</a>${item.author ? " by " + item.author : ""}</li>`).join("")}
          ${res.generalInfo.length ? "</ul>" : ""}
            </div>
            <div class="one-third">
              <h2 class="search-overlay__section-title">Programs</h2>
              ${res.programs.length ? '<ul class="link-list min-list">' : `<p>No programs found <a href="${univData.root_url}/programs">View all programs.</a></p>`}
          
          ${res.programs.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
          ${res.programs.length ? "</ul>" : ""}



              <h2 class="search-overlay__section-title">Professors</h2>
              ${res.professors.length ? '<ul class="link-list min-list">' : "<p>No professors found</p>"}
          ...
          ${res.professors.length ? "</ul>" : ""}
            ...
            <div class="one-third">
              ...
              ${res.campuses.length ? '<ul class="link-list min-list">' : `<p>No campuses found <a href="${univData.root_url}/campus">View all campuses.</a></p>`}
          
          ${res.campuses.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
          ${res.campuses.length ? "</ul>" : ""}

              <h2 class="search-overlay__section-title">Events</h2>
              ${res.events.length ? "" : `<p>No events found <a href="${univData.root_url}/events">View all events.</a></p>`}
